...
function deleteProvider(obj){
	$.ajax({
		type:"POST",
		url:path+"/providers.json",
		data:{proid:obj.attr("proid")},
		dataType:"json",
		success:function(data){
			if(data.delResult == "aaa"){//删除成功：移除删除行
				alert("whatfuck");
				cancleBtn();
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		//详情
		var obj = $(this);
		window.location.href=path+"/providerview.html/"+obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/providermodify.html/"+obj.attr("proid");
	});
